      ...
      .then((result) => {
        if (result.success) {
          ...
          const postAuthorData = () => {
            axios
              .post(
                "https://bookshelf-server-s8lf.onrender.com/add-book",
                productInfoData
              )
              .then((data) => {
                ...
                <div className="w-full">
                  ...
                  {userRole === "author" ? (
                    <input
                      type="text"
                      disabled
                      defaultValue={getUser[0]?.user_name}
                      className="input-bordered rounded-[4px] text-gray-400 border-[1px] border-[#DBDBDB] p-[7px] placeholder:text-[14px] bg-grey-600 w-full mt-2"
                    />
                  ) : (
                    <Select
