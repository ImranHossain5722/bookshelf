...
const ProductToReview = () => {
  ...
  order?.map(
    (order) => order?.delivered_status === true && deliveredOrders.push(order)
  );

  let orderedBooks = [];
  deliveredOrders?.map((order) =>
    order?.ordered_items?.map((o) => orderedBooks.push(o))
  );
              ...
              {orderedBooks?.map((book) => (
                <tr>
                  ...
                  <td className=" border-[#e1e2e6] text-[#00124E] font-bold">
                    <button
                      onClick={() => handleAddReview(book.book_id?._id)}
                      title="Add Review"
                      className=" btn btn-xs btn-error text-white  mb-3 rounded mr-1"
                    >
                      <FaCommentAlt className="font-bold text-[16px] " />
                    </button>
                  </td>
                </tr>
              ))}
