...
const AllOrders = () => {
  ...
  console.log(orders);
  useEffect(() => {
    fetch("https://bookshelf-server-s8lf.onrender.com/all-orders")
      .then((res) => res.json())
              ...
              {orders?.map((orderDetails) => (
                <tr>
                  ...
                  <td>
                    <button
                      title="Track Order"
                      onClick={() => viewOrderTrack(orderDetails._id)}
                      className=" btn btn-xs btn-primary text-white  mb-3 rounded mr-1"
                    >
                      <BsTruckFlatbed className="font-bold text-[16px] " />
                    </button>
                  </td>
                </tr>
              ))}
