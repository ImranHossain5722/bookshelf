...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://bookshelf-server-s8lf.onrender.com/all-authors", options)
      .then((response) => response.json())
  ...
  useEffect(() => {
    ...
    fetch("https://bookshelf-server-s8lf.onrender.com/all-publishers", options)
      .then((response) => response.json())
  ...
  const getChoosenCategory = (choice) => {
    setBookCat(
      Array.isArray(choice) ? choice.map((x) => ({ category_id: x.value })) : []
    );
  };
  ...
  if (book) {
    book[0]?.book_category?.map((cat) => {
      let dropDown = {
        label: cat?.category_id["category_title"],
        value: cat?.category_id["_id"],
      };
  ...
  const onSubmit = (data) => {
    ...
    const updateData = (updateData) => {
      axios
        .patch(
          `https://bookshelf-server-s8lf.onrender.com/update-book?bid=${bookid}`,
          updateData
        )
        .then((data) => {
          console.log(data);
        ...
        .then((result) => {
          if (result.success) {
            ...
            const productInfoData = {
              ...
              book_publisher:
                bookPub.length > 0 ? bookPub : book[0]?.book_publisher?._id,
              book_author:
                bookAut.length > 0 ? bookAut : book[0]?.book_author?._id,
      ...
      {isLoading ? (
        ...
        <div className="mt-1 md:w-5/6 mx-8 md:mx-auto bg-white p-10 rounded-xl shadow-md">
          <form className="" onSubmit={handleSubmit(onSubmit)}>
            <div className="md:flex">
              <div className="md:w-[30%]">
                ...
                <div className="previewProfilePic">
                  <img
                    className="playerProfilePic_home_tile max-w-[100%] mx-auto max-h-[auto] rounded-3xl"
                    src={imgData ? imgData : book[0]?.book_cover_photo_url}
                    alt=""
                  />
                </div>
              </div>
              <div className="md:w-[70%] mt-12 md:mt-0  md:ml-12">
                <div className="md:flex gap-7">
                  ...
                  <div className="w-full mt-4 md:mt-0">
                    ...
                    <Select
                      ...
                      isMulti
                      onChange={(choice) => getChoosenCategory(choice)}
                      name="categoryName"
                ...
                <div className="md:flex gap-7 mt-4">
                  <div className="w-full">
                    ...
                    <Select
                      defaultValue={{
                        label: book[0]?.book_author?.author_name,
                        value: book[0]?.book_author?._id,
                      }}
                      onChange={(choice) => getChoosenAuthor(choice)}
                      className="mt-2 rounded-xl"
                  ...
                  <div className="w-full mt-4 md:mt-0">
                    ...
                    <Select
                      defaultValue={{
                        label: book[0]?.book_publisher?.publisher_name,
                        value: book[0]?.book_publisher?._id,
                      }}
                      onChange={(choice) => getChoosenPublisher(choice)}
                      className="mt-2 rounded-xl"
                ...
                <div className="mt-4">
                  ...
                  {errors?.book_description && (
                    <p>
                      <small className="pl-1 text-red-600">
                        {errors?.book_description?.message}
                      </small>
                    </p>
                  )}
                </div>
            ...
            <input
              type="submit"
              className="mt-4 btn btn-primary w-1/2 text-white ml-[350px]"
              value="Update Book"
            />
          </form>
        </div>
