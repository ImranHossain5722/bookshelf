        ...
        <div className=" mx-auto">
          <div className="overflow-x-auto">
            <table className="table table-compact w-full">
              ...
              <tbody>
                {books[0]?.books_list?.map((book, index) => (
                  <tr key={book?._id}>
                    <th className="border border-[#666666]">{index + 1}</th>
                    <th className="border border-[#666666]">
                      {book?.book_title}
                    </th>
                    <td className="border border-[#666666]">
                      <img src={book?.book_cover_photo_url} width={50} alt="" />
                    </td>
                    <td className="border border-[#666666]">
                    ...
                    <td className="border border-[#666666]">
                      <button
                        onClick={() => handleUpdateProduct(book?._id)}
                        className="btn btn-primary text-xl mr-1 text-white"
                      >
                        <BsPencilSquare />
                      </button>
                    </td>
            ...
            {books[0]?.books_list.length === 0 && (
              <p className="text-error text-2xl text-center py-8">
                ...
                <Link
                  className="btn-primary text-xl text-white p-3 ml-4 rounded-xl"
