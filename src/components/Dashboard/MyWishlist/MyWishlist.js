...
const MyWishlist = () => {
    ...
    const deleteWishlist = (id) => {
        ...
        console.log(cartId)
        if (id) {
            axios.delete(`https://book-shelf-webapp.herokuapp.com/remove-from-wishlist?wid=${cartId}`).then(data => console.log(data))
        }
    }
  ...
  useEffect( () => {
   ...
   await axios.get(`https://book-shelf-webapp.herokuapp.com/get-wishlist-data?id=${userId}`).then(data => dispatch(whistlist(data.data[0].user_wishlist)))
    ...
    return (
        <div className="my-5">
        <p><FaRegHeart className='text-5xl text-primary mx-auto' /></p>
