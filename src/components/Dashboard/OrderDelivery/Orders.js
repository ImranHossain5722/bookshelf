...
const Orders = () => {
  ...
  let newOrders = [];
  allOrders?.map(
    (order) => order?.picked_status === false && newOrders.push(order)
  );
  ...
  const pickOrder = (id) => {
    ...
    fetch(
      `https://bookshelf-server-s8lf.onrender.com/update-order-tracking?oid=${id}`,
      {
        method: "PATCH",
        headers: {
    ...
    // axios.patch(`https://bookshelf-server-s8lf.onrender.com/update-order-tracking?oid=${id}`, pickedData)
    //     .then(data => {
                ...
                {newOrders?.map((order) => (
                  <tr key={order?._id}>
                    ...
                    <td className="border border-[#666666]">
                      ...
                      >
                        <BsTruckFlatbed />{" "}
                        <span className="ml-1">Pick order</span>{" "}
                      </button>
                    </td>
                  </tr>
                ))}
