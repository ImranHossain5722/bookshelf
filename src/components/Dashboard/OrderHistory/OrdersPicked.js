...
const OrdersPicked = () => {
    ...
    order?.map(order => (order?.picked_status === true && order?.delivered_status === false) && pickedOrders.push(order))
            ...
            <div className="w-full p-5">
                <div className="overflow-auto  h-[460px]">
                    <table className="table w-full ">
                        ...
                        {pickedOrders.length === 0 ? <Loading></Loading> :
                            <tbody className="">
                                {pickedOrders?.map((orderDetails) => <tr >
