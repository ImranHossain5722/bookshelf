...
const SearchModal = ({ showModal, setShowModal }) => {
  ...
  const handleSearch = (search) => {
    ...
    if (search.length > 0) {
      axios
        .get(`https://bookshelf-server-s8lf.onrender.com/search?sq=${search}`)
        .then((res) => {
  ...
  return (
    <div
      id="search-modal"
      className="min-h-screen w-full flex justify-center items-center absolute right-full z-50 bg-[#00124ec0] duration-500"
    >
        ...
        <div className="form-control duration-300">
          <div className="input-group">
            <input
              name="search"
              type="text"
              placeholder="Search that you desire..."
        ...
        {!error && searchResults && (
          <div className="mt-8 max-h-60 overflow-y-scroll">
            {searchResults?.map((results) => (
              <NavLink to={`/selectedBook/${results._id}`}>
                <div
                  key={results._id}
                  onClick={() => closeSearchModal()}
                  className="flex items-center justify-start h-16 w-full bg-white hover:bg-primary hover:text-white duration-200 rounded-lg mb-3 pl-4"
                >
                  <img
                    src={results.book_cover_photo_url}
                    className="h-14 mr-5"
                    alt=""
                  />
